...
/**
 * Pack and unpack one ring element with uniform coefficients. This branch
 * keeps polynomials in coefficient representation, so an element is one
 * polynomial of DEGREE coefficients and no CRT round trip is involved.
 */
void serial_put_uniform(bitwriter_t *w, nmod_poly_t a);
void serial_get_uniform(bitreader_t *r, nmod_poly_t a);

/**
 * Pack and unpack one ring element whose coefficients are short, written
 * centred.
 */
void serial_put_gauss(bitwriter_t *w, nmod_poly_t a, ulong sigma);
void serial_get_gauss(bitreader_t *r, nmod_poly_t a, ulong sigma);
